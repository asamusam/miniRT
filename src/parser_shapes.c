/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parser_shapes.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: asamuilk <asamuilk@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/01 19:51:05 by asamuilk          #+#    #+#             */
/*   Updated: 2024/05/02 19:30:55 by asamuilk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parser.h"

static int	add_node(void *content, t_list **lst)
{
	t_list	*node;

	node = ft_lstnew(content);
	if (!node)
		return (FAIL);
	ft_lstadd_back(lst, node);
	return (SUCCESS);
}

int	parse_sphere(char *line, t_data *data)
{
	int			i;
	int			status;
	t_sphere	*sphere;

	i = 0;
	status = SUCCESS;
	sphere = malloc(sizeof(t_sphere));
	if (!sphere)
		return (error_and_fail(SP, NULL, strerror(errno)));
	skip_space(&line[i], &i, NOT_REQUIRED);
	if (parse_tuple(&sphere->center, POINT, &line[i], &i) == FAIL || \
		skip_space(&line[i], &i, REQUIRED) == FAIL)
		status = error_and_fail(SP, "Position", FORMAT_ERR);
	if (parse_float(&sphere->diameter, &line[i], &i) == FAIL || \
		skip_space(&line[i], &i, REQUIRED) == FAIL)
		status = error_and_fail(SP, "Diameter", FORMAT_ERR);
	if (parse_color(&sphere->color, &line[i], &i) == FAIL)
		status = error_and_fail(SP, "Color", FORMAT_ERR);
	if (check_color(&sphere->color) == FAIL)
		status = error_and_fail(SP, "Color", RANGE_ERR);
	skip_space(&line[i], &i, NOT_REQUIRED);
	if (line[i])
		status = error_and_fail(SP, NULL, EXTRA_ERR);
	if (add_node(sphere, &data->scene->spheres) == FAIL)
		status = error_and_fail(SP, NULL, strerror(errno));
	return (check_shape_status(sphere, status));
}

int	parse_plane(char *line, t_data *data)
{
	int			i;
	int			status;
	t_plane		*plane;

	i = 0;
	status = SUCCESS;
	plane = malloc(sizeof(t_plane));
	if (!plane)
		return (error_and_fail(PL, NULL, strerror(errno)));
	skip_space(&line[i], &i, NOT_REQUIRED);
	if (parse_tuple(&plane->point, POINT, &line[i], &i) == FAIL || \
		skip_space(&line[i], &i, REQUIRED) == FAIL)
		status = error_and_fail(PL, "Position", FORMAT_ERR);
	if (parse_tuple(&plane->normal, VECTOR, &line[i], &i) == FAIL || \
		skip_space(&line[i], &i, REQUIRED) == FAIL)
		status = error_and_fail(PL, "Normal", FORMAT_ERR);
	if (check_normalized_vector(&plane->normal) == FAIL)
		status = error_and_fail(PL, "Normal", RANGE_ERR);
	if (parse_color(&plane->color, &line[i], &i) == FAIL)
		status = error_and_fail(PL, "Color", FORMAT_ERR);
	if (check_color(&plane->color) == FAIL)
		status = error_and_fail(PL, "Color", RANGE_ERR);
	if (skip_space(&line[i], &i, NOT_REQUIRED) && line[i])
		status = error_and_fail(PL, NULL, EXTRA_ERR);
	if (add_node(plane, &data->scene->planes) == FAIL)
		status = error_and_fail(PL, NULL, strerror(errno));
	return (check_shape_status(plane, status));
}

static int	parse_cylinder_info(t_cylinder *cylinder, char *line, int *i)
{
	if (parse_tuple(&cylinder->center, POINT, &line[*i], i) == FAIL || \
		skip_space(&line[*i], i, REQUIRED) == FAIL)
		return (error_and_fail(CY, "Position", FORMAT_ERR));
	if (parse_tuple(&cylinder->axis, VECTOR, &line[*i], i) == FAIL || \
		skip_space(&line[*i], i, REQUIRED) == FAIL)
		return (error_and_fail(CY, "Axis", FORMAT_ERR));
	if (check_normalized_vector(&cylinder->axis) == FAIL)
		return (error_and_fail(CY, "Axis", RANGE_ERR));
	if (parse_float(&cylinder->diameter, &line[*i], i) == FAIL || \
		skip_space(&line[*i], i, REQUIRED) == FAIL)
		return (error_and_fail(CY, "Diameter", FORMAT_ERR));
	if (parse_float(&cylinder->height, &line[*i], i) == FAIL || \
		skip_space(&line[*i], i, REQUIRED) == FAIL)
		return (error_and_fail(CY, "Height", FORMAT_ERR));
	if (parse_color(&cylinder->color, &line[*i], i) == FAIL)
		return (error_and_fail(CY, "Color", FORMAT_ERR));
	if (check_color(&cylinder->color) == FAIL)
		return (error_and_fail(CY, "Color", RANGE_ERR));
	return (SUCCESS);
}

int	parse_cylinder(char *line, t_data *data)
{
	int			i;
	int			status;
	t_cylinder	*cylinder;

	i = 0;
	status = SUCCESS;
	cylinder = malloc(sizeof(t_cylinder));
	if (!cylinder)
		return (error_and_fail(CY, NULL, strerror(errno)));
	skip_space(&line[i], &i, NOT_REQUIRED);
	if (parse_cylinder_info(cylinder, line, &i) == FAIL)
		status = FAIL;
	if (skip_space(&line[i], &i, NOT_REQUIRED) && line[i])
		status = error_and_fail(CY, NULL, EXTRA_ERR);
	if (add_node(cylinder, &data->scene->cylinders) == FAIL)
		status = error_and_fail(CY, NULL, strerror(errno));
	return (check_shape_status(cylinder, status));
}
